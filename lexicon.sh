...
option|c|category|use category|laravel
option|f|folder|folder for posts|_docs
param|1|action|action to perform: new/publish/serve
param|1|title|title of the new content
" | grep -v '^#'
}
...
new_term(){
  slug=$(lcase "$1" | sed 's/ /_/g')
  outfile="$folder/$slug.md"
  category="laravel"
  ...
  if [[ ! -f "$outfile" ]] ; then
    (
      ...
      echo "subtitle: "
      echo "description: "
      echo "image: /assets/img/unsplash/route.jpg"
    ...
    out "[$outfile] is created"
  else
    die "Post [$outfile] already exists!"
  fi
...
verify_programs(){
  ...
  verify_cache="$tmpd/.$prog_prefix.$hash_programs.verified"
